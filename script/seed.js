...
async function seed() {
  ...
  const user1 = await User.create({username: 'cody@email.com', password: '123'})
  const user2 = await User.create({
    username: 'murphy@email.com',
    password: '123'
  })
  ...
  const challenge1 = await Challenge.create({
    ...
    hint: 'Check for closing brackets (Line 3)',
  ...
  const challenge2 = await Challenge.create({
    ...
    hint: 'Check for comparison and shorthands operators (Lines 10, 11, 13)',
  ...
  const challenge3 = await Challenge.create({
    ...
    hint: 'Vin wants to tweet the "third" element! (Lines 4 & 5)',
    level: 0,
  ...
  const challenge4 = await Challenge.create({
    ...
    hint: 'Check for the grammar! :-) (Lines 4, 5)',
    level: 0,
